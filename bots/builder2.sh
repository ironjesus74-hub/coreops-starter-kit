...
# =============================================================
# CoreOps — Builder Bot 2 (Wrappers & Developer Scripts)
# Builds wrapper tools one at a time, never duplicating,
# placing completed scripts in ~/Documents/CoreOps-Factory/wrappers/
# =============================================================
set -uo pipefail
...
source "$BOTS_HOME/tools/wrappers.sh"

BUILD_INTERVAL=60   # seconds between builds
...
bot_log_info "Builder Bot 2 online (PID $$)"
bot_log_info "Factory dir: ${FACTORY_DIR}/wrappers"
...
while true; do
  ...
  # Pick the next unbuilt wrapper tool
  next_tool="$(wrap_tool_next_unbuilt)"

  if [ -z "$next_tool" ]; then
    bot_log_ok "All wrapper tools are built. Resting for ${IDLE_INTERVAL}s."
    sleep "$IDLE_INTERVAL"
    continue
  fi
  ...
  # Build it
  if outpath="$(wrap_build_tool "$next_tool")"; then
